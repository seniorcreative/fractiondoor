...
const LABEL_MODES = [
  ...
  { id: "name", label: "Names", hint: "half, third, quarter\u2026" },
  {
    id: "value",
    label: "Values",
    hint: "What one piece is worth in the current whole",
  },
];
...
/** "1/12", or "1 whole" for the single-bar end of the slider. */
  ...
  return (
    <div className="fw-panel">
      <section className="fw-group">
        ...
        {ui.unitId === "custom" && (
          <div className="fw-row">
            <label
              className="fw-field fw-field--tight"
              htmlFor="fw-custom-amount"
            >
              <span>Amount</span>
              <input
                autoComplete="off"
                ...
                onChange={(event) =>
                  patch({ customAmount: event.target.value })
                }
              />
            </label>
            <label
              className="fw-field fw-field--tight"
              htmlFor="fw-custom-unit"
            >
              <span>Unit</span>
              <input
                autoComplete="off"
            ...
            >
              ...
              <input
                ...
                placeholder="bag"
                onChange={(event) =>
                  patch({ customWholeName: event.target.value })
                }
              />
            </label>
          </div>
        ...
        />
        <p className="fw-hint">
          Two wholes and a half block make 2&nbsp;1/2. Click pieces in different
          wholes to build mixed numbers.
        </p>
      </section>
      ...
      <section className="fw-group">
        ...
        {ui.rowPreset === "custom" ? (
          <label className="fw-field" htmlFor="fw-custom-rows">
            ...
            <input
              ...
              onChange={(event) =>
                patch({ customDenominators: event.target.value })
              }
            />
          </label>
        ) : (
          <label className="fw-field" htmlFor="fw-max-den">
            <span>
              Deepest row <strong>{rowLabel(ui.maxDenominator)}</strong>
            </span>
            <input
              autoComplete="off"
              id="fw-max-den"
              ...
              onChange={(event) =>
                patch({ maxDenominator: Number(event.target.value) })
              }
            />
          </label>
        )}
        ...
        <p className="fw-stat">
          {denominators.length} {denominators.length === 1 ? "row" : "rows"}{" "}
          &middot; {pieceCount.toLocaleString()}{" "}
          {pieceCount === 1 ? "block" : "blocks"} &middot; deepest row{" "}
          {rowLabel(deepest)}
        </p>
        {heavy && (
          <p className="fw-warn">
            That is a lot of blocks. It still runs in one draw call, but labels
            thin out and older machines may slow down.
          </p>
        )}
      </section>
